...
$columnas = ["id", "Almacen", "PersonaR", "Telefono", "Correo", "Ubicacion"];
...
if ($filas > 0) {
	while($fila = $respuesta->fetch_assoc()){
		...
		$salida["data"].='<td>'.$fila['PersonaR'] .'</td>';
		$salida["data"].='<td>'.$fila['Telefono'] .'</td>';
		$salida["data"].='<td>'.$fila['Correo'] .'</td>';
		$salida["data"].='<td>'.$fila['Ubicacion'] .'</td>';

						$salida["data"].='<td> <a class="enlace" href="/Almacen/editar/editar_almacen.php?id='.$id.'"> <abbr title="Editar"><span class="icono">&#9998;</span></abbr></a>  ';
					

					$salida["data"].= '
								<a class="enlace" href = "/Almacen/eliminar/eliminar_almacen.php?id='.$id.'" > <abbr title="Eliminar"> <span class="icono">&#10006;</span></abbr> </a> 
							  </td>';
